...
class SearchGlobal(Scaffold):
    async def search_global(
        ...
        If you want to get the messages count only, see :meth:`~pyrogram1.Client.search_global_count`.
        ...
        Returns:
            ``Generator``: A generator yielding :obj:`~pyrogram1.types.Message` objects.

        Example:
            .. code-block:: python

                # Search for "pyrogram1". Get the first 50 results
                for message in app.search_global("pyrogram1", limit=50):
                    print(message.text)
