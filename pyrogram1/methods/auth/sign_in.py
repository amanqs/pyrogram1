...
class SignIn(Scaffold):
    async def sign_in(
        ...
        Parameters:
            ...
            phone_code_hash (``str``):
                Code identifier taken from the result of :meth:`~pyrogram1.Client.send_code`.

            phone_code (``str``):
                The valid confirmation code you received (either as Telegram message or as SMS in your phone number).

        Returns:
            :obj:`~pyrogram1.types.User` | :obj:`~pyrogram1.types.TermsOfService` | bool: On success, in case the
            authorization completed, the user is returned. In case the phone number needs to be registered first AND the
            terms of services accepted (with :meth:`~pyrogram1.Client.accept_terms_of_service`), an object containing
