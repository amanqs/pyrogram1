...
class Dispatcher:
    ...
    def __init__(self, client: "pyrogram1.Client"):
        self.client = client
        ...
        async def message_parser(update, users, chats):
            return await pyrogram1.types.Message._parse(
        ...
        async def callback_query_parser(update, users, chats):
            return await pyrogram1.types.CallbackQuery._parse(self.client, update, users), CallbackQueryHandler

        async def user_status_parser(update, users, chats):
            return pyrogram1.types.User._parse_user_status(self.client, update), UserStatusHandler

        async def inline_query_parser(update, users, chats):
            return pyrogram1.types.InlineQuery._parse(self.client, update, users), InlineQueryHandler

        async def poll_parser(update, users, chats):
            return pyrogram1.types.Poll._parse_update(self.client, update), PollHandler

        async def chosen_inline_result_parser(update, users, chats):
            return pyrogram1.types.ChosenInlineResult._parse(self.client, update, users), ChosenInlineResultHandler

        async def chat_member_updated_parser(update, users, chats):
            return pyrogram1.types.ChatMemberUpdated._parse(self.client, update, users, chats), ChatMemberUpdatedHandler

        async def chat_join_request_parser(update, users, chats):
            return pyrogram1.types.ChatJoinRequest._parse(self.client, update, users, chats), ChatJoinRequestHandler
                ...
                async with lock:
                    for group in self.groups.values():
                        for handler in group:
                            ...
                            except pyrogram1.StopPropagation:
                                raise
                            except pyrogram1.ContinuePropagation:
                                continue
                            except Exception as e:
                                log.error(e, exc_info=True)
